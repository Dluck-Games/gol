...
Foreman is a task runner for D.L.K — 栞 calls Foreman, Foreman calls a coding agent. It replaces manual `opencode` / `CodeBuddy` invocations with templated, tracked, reproducible task execution. Clients are configured in `config.yaml` with env-file-driven model switching (e.g., `cc-glm`, `cc-kimi`, `cc-internal`).
...
| `cc-glm` | `CodeBuddy` | `.env.ccg` | CodeBuddy Code via GLM provider |
| `cc-kimi` | `CodeBuddy` | `.env.cck` | CodeBuddy Code via Kimi provider |
| `cc-internal` | `CodeBuddy` | `.env.cci` | CodeBuddy Code via internal provider |
...
CodeBuddy_CODE_DISABLE_1M_CONTEXT=1
```
...
3. **Skill existence:** For each task's `skills` list — check `.claude/skills/{name}/SKILL.md` exists (CodeBuddy Code skills) or note plugin skills (`superpowers:*`)
