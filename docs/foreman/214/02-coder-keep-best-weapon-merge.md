...
- **`scripts/components/c_weapon.gd`** — `CWeapon.on_merge()` 从全量覆盖改为 per-field keep best：`interval`/`spread_degrees` 取 min，`bullet_speed`/`attack_range` 取 max，`bullet_recipe_id` 保持全量覆盖
- **`scripts/components/c_melee.gd`** — `CMelee.on_merge()` 同理：`attack_interval`/`swing_duration` 取 min，`damage`/`attack_range`/`ready_range`/`swing_angle` 取 max，`night_attack_speed_multiplier` 保持全量覆盖
...
4. **修复 test_cold_rate_conflict**：该测试原本验证"merge 后 cost system 重新捕获 base"，将测试数据改为 incoming 更优的场景，同时验证 keep-best 和 cost system 交互
