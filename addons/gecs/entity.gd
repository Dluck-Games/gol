...
func _ready() -> void:
	# initialize()
	initialize_gol() # avoid change props in editor @dluck

func _process(delta: float) -> void:
	pass
			
# @dluck - Disable serialization of components and relationships, we create it at runtime
...
func initialize_gol():
	_entityLogger.trace('_ready Entity Initializing Components: ', self)
	for res_component in component_resources:
		add_component(res_component.duplicate(true))

	for def_component in define_components():
		if not has_component(def_component.get_script()):
			add_component(def_component.duplicate(true))
