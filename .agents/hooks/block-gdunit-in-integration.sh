...
if hook_patch_adds_text_in_path "$INPUT" "tests/playtest/" "extends GdUnitTestSuite"; then
  echo "BLOCKED: GdUnitTestSuite not allowed in tests/playtest/ (use AutomationPlayTestSuite instead)" >&2
  exit 2
fi
