...
│   └── pixel-art/                 # AI pixel art pipeline: GPT handoff/ComfyUI → normalize → evaluate
...
**Playtest video analysis:** When a recorded playtest MP4 needs visual review, read the text report/log first, then analyze the video. Use native video input when the selected model supports it (Gemini 2.5 Pro/Flash are preferred low-cost choices); otherwise extract frames with `ffmpeg` into `.debug/video-frames/<suite>/` and send ordered frames to the vision model. Use 1-2 FPS for normal review and 3-5 FPS for fast UI, combat, or short-lived rendering issues. See `gol-debug` and `gol-test-runner` skills for the standard commands.
